...
class Binance(exchanges.SpotCCXTExchange):
    ...
    def get_additional_connector_config(self):
        config = {}
        proxy = self.config['exchanges']['binance'].get('proxy', None)
        if proxy:
            config['proxies'] = {
                'http': proxy,
                'https': proxy
    ...
    async def get_my_recent_trades(self, symbol=None, since=None, limit=None, **kwargs):
        return self._uniformize_trades(
            await super().get_my_recent_trades(
                symbol=symbol,
                since=since,
                limit=limit,
                **kwargs
    ...
    async def create_order(
        self, order_type: trading_enums.TraderOrderType, symbol: str, quantity: decimal.Decimal,
        price: decimal.Decimal = None, stop_price: decimal.Decimal = None,
        side: trading_enums.TradeOrderSide = None, current_price: decimal.Decimal = None,
        params: dict = None
    ):
        return await self._ensure_order_completeness(
            await super().create_order(
